...
public class 筛选装备 {
    ...
    public static void savePicture(int x, int y, Robot robot) {
        ...
        try {
            ...
            System.out.println("OutPicture/screenshot" + LocalTime.now().format(DateTimeFormatter.ofPattern("HHmmss")) + ".png");
...
//            Thread.sleep(clip.getMicrosecondLength() / 1000);
    ...
    public static void run(int x, int y, Robot robot, 筛选装备_子类 筛选装备_子类, String[] 要的词缀, String[] 不要的词缀,int 有效词条要求) {
        ...
        while (是否筛选装备 == true) {
            ...
            AtomicReference<String> 类别 = new AtomicReference<>("");
            try {
                // 构建命令
                ...
                List<String> result1 = result.subList(物品强度索引 + 1, 装备时损失属性索引);
                System.out.println(result1);

                for (String s : result1) {
                    if (类别.get().equals(装备种类.get(0))) {
                    ...
                    } else if(类别.get().equals(装备种类.get(2))){
                        for (String s1 : 要的词缀) {
            ...
            System.out.println("-----需求词条数量: " + 需求词条数量);
            System.out.println("-----数值优秀: " + 数值优秀);
